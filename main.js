...
function charpterFromLink(browser) {
    return async function(link) {
        ...
        const title = await page.$eval('#content h1', title => title.innerText)
...
async function linksFromUrl(browser, url) {
    ...
    const links = await page.$$eval('div.centent ul li a', (ls, baseUrl) => {
...
async function download(url, concurrency = 20) {
    ...
    for await (const value of asyncPool(concurrency, links, charpterFromLink(browser))) {
